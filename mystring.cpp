...
#define _CRT_SECURE_NO_WARNINGS	//used this to prevent depreciation while using strcpy on my compiler error on VS
...
//Function that will be used to format data 
////////////////////////////////////////////////////////////////////////////
void my_string::formatData(int colums)
{
	for (int i = colums; i < current_length; i++)
	{
		if (sequence[i] == '-')											//if at index i theres a hyphen in sequence[i]
		{
			if (sequence[i + 1] == ' ')									//it will go to sequence[i+1] and check for a hyphen
			{
				sequence[i + 1] = '\n';									//if true endline character will be added to that index
			}

		}
		else
			sequence[i] = '\n';											//else endline will be added at sequence[i]
		i + (colums - 1);											//will go the next line of data
	}
